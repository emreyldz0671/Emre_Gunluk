#!/usr/bin/env python3
"""
WaveTrend GEÇMİŞ Sinyal Tarayıcı — TradingView Verisi
Son işlem gününü otomatik bulur
"""
...
def sinyal_var_mi_idx(wt1, wt2, hedef_idx):
    cross_count = 0
...
def gecmis_tara():
    ...
    print(f"\n{'='*50}")
    print(f"GEÇMİŞ Tarama — Cuma Günü Sinyalleri")
    ...
    for i, sembol in enumerate(BIST_SEMBOLLER):
        try:
            ...
            # Son işlem günü = son bar (cuma)
            hedef_idx = n - 1
            tarih = df.index[hedef_idx].strftime("%d.%m.%Y")
            kapanis = round(float(df['close'].iloc[hedef_idx]), 2)

            # YEŞİL sinyal: wt1 wt2'yi yukarı kesiyor + -45 altında + 3. kesişim
            if sinyal_var_mi_idx(wt1, wt2, hedef_idx):
                yesil_sinyaller.append({"sembol": sembol, "kapanis": kapanis})
                print(f"  🟢 YEŞİL: {sembol}")

            # KIRMIZI sinyal: wt1 wt2'yi aşağı kesiyor (bearish cross)
            bear_cross = (wt1.iloc[hedef_idx-1] > wt2.iloc[hedef_idx-1]) and \
                         (wt1.iloc[hedef_idx] < wt2.iloc[hedef_idx])
            if bear_cross:
                kirmizi_sinyaller.append({"sembol": sembol, "kapanis": kapanis})
                print(f"  🔴 KIRMIZI: {sembol}")
    ...
    # Telegram mesajı
    mesaj = f"📊 <b>Son İşlem Günü Sinyalleri</b>\n📅 {simdi}\n{'─'*25}\n\n"

    if yesil_sinyaller:
        mesaj += f"🟢 <b>YEŞİL SİNYAL ({len(yesil_sinyaller)} hisse)</b>\n"
        for s in yesil_sinyaller:
            mesaj += f"📌 <b>{s['sembol']}</b> — {s['kapanis']} ₺\n"
        mesaj += "\n"

    if kirmizi_sinyaller:
        mesaj += f"🔴 <b>KIRMIZI SİNYAL ({len(kirmizi_sinyaller)} hisse)</b>\n"
